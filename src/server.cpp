...
void Server::ProcessData(u8 *data, u32 datasize, u16 peer_id)
{
	...
	if(command == TOSERVER_INIT)
	{
		...
		*/
		...
		int connecting =
				m_clients.getClientIDs(DefinitionsSent).size() -
				m_clients.getClientIDs().size();
		connecting = MYMAX(0,connecting);
		//Enforce max simultaneous join limit
		if (connecting >= g_settings->getU16("max_concurrent_joins"))
		{
			actionstream<<"Server: "<<playername<<" tried to join, but there"
					<<" are already " << connecting << " users connecting right now."
					<<std::endl;
			DenyAccess(peer_id, L"Sorry, too many concurrent connects try again in a few minutes.");
			return;
		}

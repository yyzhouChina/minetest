...
bool TCPPeer::Ping(float dtime,SharedBuffer<u8>& data)
{
	...
	if((m_ping_timer >= PING_TIMEOUT) || (m_tcp_ping_timer >= 5))
	{
...
void TCPPeer::reportRTT(float rtt)
{
	if (rtt < 0.0) {
		...
		avg_rtt = rtt * 0.005 + avg_rtt * 0.995;
}
...
void Connection::putCommand(ConnectionCommand &c)
{
	if (c.reliable)
	{
		if ( (c.peer_id == 0) || (c.peer_id == PEER_ID_SERVER) ) {
			if (g_settings->getBool("client_connect_tcp"))
			{

...
bool TCPPeer::sendTCPPacket(SharedBuffer<u8> tosend)
{
	if (!isActive())
		return false;
	...
	char packetbuffer[1360];

	unsigned int bufferfill = 0;

	packetbuffer[bufferfill] = TCP_PACKET_START;
	bufferfill++;

	for(unsigned int byte = 0; byte < tosend.getSize(); byte++) {
		char sendchar = tosend[byte];
		if ((sendchar == TCP_PACKET_START) ||
			(sendchar == TCP_PACKET_END) ||
			(sendchar == TCP_PACKET_ESCAPE)) {
			packetbuffer[bufferfill] = TCP_PACKET_ESCAPE;
			bufferfill++;
		}

		packetbuffer[bufferfill] = sendchar;
		bufferfill++;

		// send at buffersize -1 to always have enough room for a single escaped
		// byte
		if (bufferfill >= sizeof(packetbuffer)-1) {
			write(m_fd,packetbuffer,bufferfill);
			bufferfill = 0;
		}
	}

	packetbuffer[bufferfill] = TCP_PACKET_END;
	bufferfill++;
	write(m_fd,packetbuffer,bufferfill);
	return true;
}
...
SharedBuffer<u8> TCPPeer::readPacket()
{
	if (m_received_packets.size() > 0) {
		fprintf(stderr,"Have another %d packets queued\n",m_received_packets.size()-1);
...
u16 TCPPeer::getNextSplitSequenceNumber()
{
	JMutexAutoLock lock(m_exclusive_access_mutex);
	return m_splitSequenceNumber;
}

void TCPPeer::setNextSplitSequenceNumber(u16 toset)
{
	JMutexAutoLock lock(m_exclusive_access_mutex);
	m_splitSequenceNumber = toset;
}
...
void UDPPeer::reportRTT(float rtt)
{
	if (rtt < 0.0) {
		...
		avg_rtt = rtt * (1.0/(MAX_RELIABLE_WINDOW_SIZE*10)) +
			avg_rtt * (((MAX_RELIABLE_WINDOW_SIZE*10)-1.0)/(MAX_RELIABLE_WINDOW_SIZE*10));
...
void* ConnectionTCPSendThread::Thread()
{
	...
	while(!StopRequested())
	{
			...
			(m_connection->GetPeerID() != PEER_ID_INEXISTENT))
		{
			...
			ConnectionCommand c = m_connection->m_tcp_command_queue.pop_frontNE(0);
			while(c.type != CONNCMD_NONE){
				...
				if((m_connection->waitForPeerID() == false) ||
					(m_connection->GetPeerID() != PEER_ID_INEXISTENT))
				{
					c = m_connection->m_tcp_command_queue.pop_frontNE(0);
				}
				else {
					break;
				}
...
void ConnectionTCPSendThread::disconnect()
{

}

void ConnectionTCPSendThread::disconnect_peer(u16 peer_id)
{

}

void ConnectionTCPSendThread::send(u16 peer_id, u8 channelnum,
						SharedBuffer<u8> data)
{
	...
	BufferedPacket p = con::makePacket(peer->address, data,
				m_connection->GetProtocolID(), m_connection->GetPeerID(),
				channelnum);
		dynamic_cast<TCPPeer*>(&peer)->sendTCPPacket(p.data);

	peer->m_increment_packets_remaining =
		MYMIN(0, (int) peer->m_increment_bytes_remaining - (int) p.data.getSize());
}

void ConnectionTCPSendThread::sendToAll(u8 channelnum,
						SharedBuffer<u8> data)
{
...
void ConnectionTCPSendThread::sendPackets()
{
			...
			j != peerIds.end(); ++j)
	{
		...
		peer->m_increment_bytes_remaining = m_iteration_bytes/m_connection->m_peers.size();
		...
		for (unsigned int i=0; i < CHANNEL_COUNT; i++)
		{
					...
					(dynamic_cast<TCPPeer*>(&peer)->m_queued_commands[i].size() > 0))
			{
				...
				if (c.raw == true) {
					...
					send(*j,i,makeOriginalPacket(c.data));
				}
			}
		}
	}
...
void * ConnectionSendThread::Thread()
{
	...
	while(!StopRequested() || packetsQueued()) {
		...
		/* TODO dynamicaly adjust command and packet transfer values */
		/* need to find out usefull parameters first */
		...
		SharedBuffer<u8> data)
{
	...
	u16 split_sequence_number = 0;
	Channel *channel = 0;

	if (dynamic_cast<UDPPeer*>(&peer) != 0)
	{
		Channel *channel = &(dynamic_cast<UDPPeer*>(&peer)->channels[channelnum]);
		split_sequence_number = channel->readNextSplitSeqNum();
	}
	else
	{
		split_sequence_number =
				dynamic_cast<TCPPeer*>(&peer)->getNextSplitSequenceNumber();
	}
	...
	if (channel != 0) {
		channel->setNextSplitSeqNum(split_sequence_number);
	}
	else {
		dynamic_cast<TCPPeer*>(&peer)->setNextSplitSequenceNumber(split_sequence_number);
	}
...
void ConnectionTCPReceiveThread::receive()
{
	...
	for (std::list<u16>::iterator i = peerlist.begin(); i != peerlist.end(); i++)
	{
		if ((*i > MAX_UDP_PEERS) || (*i == PEER_ID_SERVER)){
			...
			tcp_fds[*i] = dynamic_cast<TCPPeer*>(&peer)->m_fd;
		}
	}
	...
	if (tcp_fds.size() > 0)
	{
		...
		for (std::map<u16,int>::iterator i = tcp_fds.begin(); i != tcp_fds.end(); i++)
		{
			PeerHelper peer = m_connection->getPeer(i->first);
		...
		SharedBuffer<u8> packetdata, u16 peer_id)
{
	...
	fprintf(stderr,"Protocol ID: %d sender_peer_id %d, channel: %d size: %d\n",
			protocolid,sender_peer_id & 0xFFFF, channel & 0xFF,packetdata.getSize());
...
void ConnectionReceiveThread::receive()
{
	...
	try{
		...
		// TODO make correct check for UDP / TCP mixed mode
		...
		u8 channelnum, bool reliable)
{
	...
	if((peer_id <= MAX_UDP_PEERS) && (type == TYPE_CONTROL))
	{
		assert(channel != 0);
	...
	else if((peer_id <= MAX_UDP_PEERS) && (type == TYPE_SPLIT))
	{
		assert(channel != 0);
	...
	while(!StopRequested()) {
		...
		else
		{
			...
			if (incoming_fd >= 0)
			{
				...
				//create peer
				u16 peer_id = m_connection->createPeer(incoming_address,TCP,incoming_fd);
			}
		}
		END_DEBUG_EXCEPTION_HANDLER(derr_con);
	}
...
void Connection::putCommand(ConnectionCommand &c)
{
	...
	{
		if (c.peer_id == 0) {
			/* TODO decide uppon setting where to send! */
			//m_command_queue.push_back(c);
			m_tcp_command_queue.push_back(c);
			return;
		}
		else if (c.peer_id == PEER_ID_SERVER)
		{
			//TODO add setting for connect mode
			if (true) m_tcp_command_queue.push_back(c);
			else m_command_queue.push_back(c);
		}
		else if (c.peer_id > MAX_UDP_PEERS)
		{
			m_tcp_command_queue.push_back(c);
		}
		else {
		...
		{
		case CONNCMD_SERVE:
			//TODO add setting for tcp + udp server
			if (true) m_command_queue.push_back(c);
			if (true) m_tcp_command_queue.push_back(c);
			break;

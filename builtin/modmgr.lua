...
function modmgr.tab()
	...
	retval = retval ..
		...
		"button[3,4.85;2,0.5;btn_mod_mgr_install_local;Download]"
...
function modmgr.handle_modmgr_buttons(fields)
	...
	if fields["mod_mgt_open_dlg_accepted"] ~= nil and
		...
		local modfile = identify_filetype(fields["mod_mgt_open_dlg_accepted"])
		...
		if basefolder.type == "modpack" then
			local clean_path = get_last_folder(cleanup_path(basefolder.path))
			...
			if clean_path ~= nil then
				...
				gamedata.errormessage = "Install Mod: unable to find suitable foldername for modpack " 
					.. fields["mod_mgt_open_dlg_accepted"]
			end
		end
		
		if basefolder.type == "mod" then
			local targetfolder = modmgr.identify_modname(basefolder.path,"init.lua")
